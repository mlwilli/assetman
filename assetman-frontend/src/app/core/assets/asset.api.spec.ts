...
describe('AssetApi', () => {
  ...
  it('listAssets calls GET /api/assets with page/size and provided filters', () => {
    ...
    const req = httpMock.expectOne((r) => r.method === 'GET' && r.url.includes('/api/assets'));
    ...
    expect(req.request.params.get('locationId')).toBe('11111111-1111-1111-1111-111111111111');

    req.flush({
      content: [{ id: 'a', tenantId: 't', name: 'Hammer Drill', status: 'IN_SERVICE' }],
      page: 2,
      size: 50,
      totalElements: 1,
      totalPages: 1,
  ...
  it('getAsset calls GET /api/assets/:id', () => {
    ...
    const req = httpMock.expectOne((r) => r.method === 'GET' && r.url.includes('/api/assets/abc'));
  ...
  it('createAsset calls POST /api/assets', () => {
    ...
    const req = httpMock.expectOne((r) => r.method === 'POST' && r.url.includes('/api/assets'));
  ...
  it('updateAsset calls PUT /api/assets/:id', () => {
    api.updateAsset('id1', { name: 'Updated', status: 'RETIRED' }).subscribe((res) => {
      expect(res.name).toBe('Updated');
    });

    const req = httpMock.expectOne((r) => r.method === 'PUT' && r.url.includes('/api/assets/id1'));
  ...
  it('deleteAsset calls DELETE /api/assets/:id', () => {
    ...
    const req = httpMock.expectOne((r) => r.method === 'DELETE' && r.url.includes('/api/assets/id2'));
    req.flush(null);
  });
